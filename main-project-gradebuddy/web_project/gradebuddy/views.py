...
def categories_for_class(request, user_id, class_id):
    ...
    if request.method == "GET":
        ...
        Category.objects.filter(associated_class=class_instance).delete()
...
@api_view(["GET", "POST", "DELETE"])
def category_assignments(request, user_id, class_id, category_id):
    # Check if the requesting user is the owner of the class
    class_instance = get_object_or_404(Class, pk=class_id, user_id=user_id)
    ...
    if request.method == "GET":
        ...
        # Delete all assignments within a category
        Assignment.objects.filter(associated_category_id=category_id).delete()
        return Response(status=204)
